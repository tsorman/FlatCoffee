...
public class FlatCoffeeGaugeDrawLabel extends JComponent 
{
	public FlatCoffeeGaugeDrawLabel() {
		super();
	}
	
	public FlatCoffeeGaugeDrawLabel(int gaugeposition, String displayMaxValue, String displayMinValue, String displayValue) {
		super();
	...
	private String displayMaxValue = "100", displayMinValue = "-100" , displayValue = "0";
	...
	public void paint ( Graphics g)
	{
		...
		if ( gaugeposition > 0 )
		{
			...
			graphics.fillRect(COMPONENT_CENTER, this.getHeight() - 15 , ((COMPONENT_CENTER * gaugeposition ) / 100) - 2 , 10 );
			graphics.fillArc(COMPONENT_CENTER + ((COMPONENT_CENTER * gaugeposition ) / 100) - 6 , this.getHeight() - 15 , 6, 10, -90, 180);
		...
		if ( gaugeposition < 0 )
		{
			double tintWeight = 0.5 - (double)(gaugeposition*-1) / 200   ;
			
			Color barColor = new Color ( (int) (NEGATIVE_COLOR.getRed() + ( 255  - NEGATIVE_COLOR.getRed() ) * tintWeight ) ,
					 (int) (NEGATIVE_COLOR.getGreen() + ( 255  - NEGATIVE_COLOR.getGreen() ) * tintWeight ) ,
					 (int) (NEGATIVE_COLOR.getBlue() + ( 255  - NEGATIVE_COLOR.getBlue() ) * tintWeight ) ) ;
			
			graphics.setColor(barColor);
			
			graphics.fillRect( COMPONENT_CENTER - ((COMPONENT_CENTER * (gaugeposition * -1) ) / 100) + 3, this.getHeight() - 15 , ((COMPONENT_CENTER * (gaugeposition * -1) ) / 100) -3 , 10 );
			graphics.fillArc(COMPONENT_CENTER + ((COMPONENT_CENTER * gaugeposition ) / 100) +1 , this.getHeight() - 15 , 6, 10, -90, -180);
		}
		...
		// LEFT CORNER LINE 
		graphics.drawLine(0, ONE_THIRD_HEIGHT , 0, this.getHeight() - 1 );
		
		// NEGATIVE HALFWAY MARKER
		graphics.drawLine(COMPONENT_CENTER/2, this.getHeight() - 4 , COMPONENT_CENTER/2 , this.getHeight() - 1 );
		
		// POSITIVE HALFWAY MARKER
		graphics.drawLine(COMPONENT_CENTER + COMPONENT_CENTER/2, this.getHeight() - 4 , COMPONENT_CENTER + COMPONENT_CENTER/2 , this.getHeight() - 1 );
		
		//RIGHT CORNER LINE
		graphics.drawLine(getWidth()-1, ONE_THIRD_HEIGHT , this.getWidth() - 1 , this.getHeight() - 1 );
		
		//CENTERLINE
		graphics.drawLine(COMPONENT_CENTER , TWO_THIRDS_HEIGHT , COMPONENT_CENTER , this.getHeight() - 1 );
		...
		{
			//DrawMin value
			graphics.drawString(displayMinValue, 1, fonthgt + 6 );
			
			//DrawMax value
			graphics.drawString(displayMaxValue, this.getWidth() - metrics.stringWidth(displayMaxValue) + 2 , fonthgt + 6);
		}
	}
}